...
import { useAuth } from "../Components/extra/authContext";
...
export default function Main() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      setIsLoading(true);
      setSuccess(true);
      const response = await fetch("https://car-rental-back.onrender.com/login", {
        method: "POST",
        headers: {
    ...
    <div className="flex justify-center items-center ">
      ...
      >
        ...
        >
          ...
          <Link
            className="flex justify-left mt-2 mb-2 text-sm hover:text-B-yellow font-bold"
            to="/ConfirmEmail"
          >
            Forgot Password?
          </Link>
        ...
        <div className="divContent ">
          <div className="flex flex-col p-4 rounded-2xl h-full bg-L-black text-justify">
            ...
            <p className="text-white">
              ...
              an Account?
              <span className="text-B-yellow font-bold"> SIGN UP</span>
            </p>
          </div>
        </div>
      </div>
